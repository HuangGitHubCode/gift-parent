<template>
	<section>
      ...
      <el-table-column prop="descs" label="权限描述" width="120" sortable>
      </el-table-column>
      <el-table-column prop="parent.name" label="父权限名称" width="180" sortable>
      </el-table-column>
      <el-table-column prop="parent.descs" label="父权限描述" width="180" sortable>
      </el-table-column>
...
<script>
	...
	export default {
      ...
      // 查询部门树
      getDeptTree(){
        this.$http.get("/permission/deptTree")
            .then(res=>{
      ...
      handleSizeChange(val){
        ...
        // 发送请求 请求后台
        this.$http.post("/permission",this.queryFrom).then(res=>{
          // console.log(res)
          res = res.data.data
          this.list = res.list
        ...
        this.getDeptTree()
          ...
          this.$http.patch("/permission",ids)
              .then(res=>{
                if (res.data.success){
                  this.$message({type:"success",message:"删除成功"})
                  this.queryFrom.currentPage = 1;
